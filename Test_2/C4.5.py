...
def ChooseBestDataSet(dataset):
    num_data = len(dataset[0]) - 1#计算一共有多少个特征，因为最后一列一般是分类结果，所以需要-1
    baseEntropy = getShang(dataset)   # 定义初始的熵,用于对比分类后信息增益的变化
    bestGainRate = 0.0    #最好的信息增益率
    bestFeature = 0
    for i in range(num_data):
        ...
        for value in uniqueVals:
            subDataset = splitDataSet(dataset, i, value)    #获取按照特征value分类后的数据
            prob = len(subDataset) / float(len(dataset))  # 计算子集与原数据集的长度比
            newEntropy += prob * getShang(subDataset)  # 按特征分类后计算得到的熵
        infogain = newEntropy - baseEntropy # 原始熵与按特征分类后的熵的差值，即信息增益
        splitonfo = getShang(subDataset) #分裂信息
        if splitonfo == 0:#若特征值相同,即splitonfo和info均为0，则跳过该特征
            continue
        GainRate = infogain/splitonfo #计算信息增益率
        if (GainRate > bestGainRate):   # 若按某特征划分后，若infoGain大于bestInf，则infoGain对应的特征分类区分样本的能力更强，更具有代表性。
            ...
            bestFeature = i #将最大的信息增益对应的特征下标赋给bestFea，返回最佳分类特征
    return bestFeature
'''
...
def ChooseBestDataSet(dataset):
    ...
    for i in range(num_data):
        ...
        infogain = newEntropy - baseEntropy # 原始熵与按特征分类后的熵的差值，即信息增益
        splitonfo = 0.0
        for prob in problist:   #分裂信息
            splitonfo -= prob * log(prob, 2)
        if splitonfo == 0:#若特征值相同,即splitonfo和info均为0，则跳过该特征
            continue
        GainRate = infogain/splitonfo #计算信息增益率
        if (GainRate > bestGainRate):   # 若按某特征划分后，若infoGain大于bestInf，则infoGain对应的特征分类区分样本的能力更强，更具有代表性。
